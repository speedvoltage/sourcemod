...
void RenameCheck(char getsteamid[64], char client)
{
	...
	if(clientAdmin != INVALID_ADMIN_ID && GetAdminFlag(clientAdmin, Admin_Generic, Access_Effective))
	{
		return;
	}
...
public Action Command_Rename(int client, int args)
{
			...
			tn_is_ml)) > 0)
	{
		...
		for (int i = 0; i < target_count; i++)
		{
			if (target_count > 1)
			{
				PrintToChat(client, "%s%s %sYou cannot rename more than one player at a time.", CTAG, TAG, CUSAGE);
				return Plugin_Handled;
			}
...
{	
	...
	hTimer[target] = CreateTimer(timeleft + 0.0, name_temp_ban, DP, TIMER_FLAG_NO_MAPCHANGE|TIMER_DATA_HNDL_CLOSE);
...
public Action name_temp_ban(Handle timer, any DP)
{
	...
	if(!newFile)
	{
		...
		PrintToServer("%s Could not open %s", TAG, adminrenamedfile);
	...
	char writeLine[32];
	for(int i, size = GetArraySize(fileArray); i < size; i++)    // size of fileArray equals to zero. LOL
	{
		fileArray.GetString(i, writeLine, sizeof(writeLine));
		newFile.WriteLine(writeLine);
		if(GetConVarBool(changename_debug))
...
{	
	...
	{		
		...
		if(strcmp(buffer, currentname, false))
		{
			if (GetConVarBool(changename_checkbadnames))
			{	
				if(playerAdmin == INVALID_ADMIN_ID || !GetAdminFlag(playerAdmin, Admin_Generic, Access_Effective))
				{
	...
	{ 
		...
		if(strcmp(sName, currentname))
		{
			...
			if(GetAdminFlag(playerAdmin, Admin_Generic, Access_Effective))
			{
				for (int z = 0; z < adminrenamedlines; z++)
				{					
					if (StrContains(steamid, adminrenamedid[z], false) != -1)
					{
						CheckTimer(client);
						if (GetConVarBool(changename_debug))
						{
